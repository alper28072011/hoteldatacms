...
const getTypeInfo = (type: string) => {
  switch (type) {
    ...
    case 'qa_pair': return { label: 'Q&A', desc: 'A specific question and answer.', icon: <HelpCircle size={14} /> };
...
const NodeEditor: React.FC<NodeEditorProps> = ({ node, root, onUpdate, onDelete }) => {
  ...
  const handleAddSubContent = (type: 'qa_pair' | 'note' | 'field') => {
      ...
      // Note: addChild in context usually adds "New Item". 
      // We rely on the tree updating and the user can then edit it. 
      // Ideally, we might want to select it immediately, but Context doesn't return ID easily yet.
  };
                        ...
                        />
                         {node.type === 'qa_pair' && (
                             <input 
                                ...
                                className="hidden" // Sync name/question for Q&A
                             />
                         )}
                    </div>
                )}
                                 ...
                                 <div key={sub.id} className="group flex items-start gap-3 p-3 hover:bg-slate-50 rounded-lg border border-transparent hover:border-slate-100 transition-all">
                                     ...
                                     <div className="flex-1 min-w-0">
                                         ...
                                         <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                             <button 
                                                onClick={() => {
                                                    // This is a bit tricky without redirecting, 
                                                    // ideally we would allow inline editing or set selectedNodeId
                                                    // For now, let's just show ID to copy or we need a way to invoke onSelect from here.
                                                    // Assuming user clicks in tree for full edit.
                                                    alert("To edit this detail fully, please select it in the tree view on the left.");
                                                }}
